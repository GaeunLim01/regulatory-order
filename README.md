...
Modules 1, 2, and 3 sit side by side as three equal panels rather than two axes plus a
band underneath. 1 and 2 are independent axes and 3 is a differently-shaped process, but
none is subordinate to the others, so the layout keeps all three as siblings and leaves
the distinction to the copy instead of to a visual hierarchy. A conventional docs sidebar
would render all three as a nested tree and quietly reassert the hierarchy the
three-module structure removed.
